...
private:
    std::shared_ptr<Area> FindOrCreateArea(DS3_OnlineAreaId AreaId)
    {
...
public:
    ...
    bool Remove(DS3_OnlineAreaId AreaId, EntryId Id)
    {
    ...
    std::shared_ptr<ValueType> Find(DS3_OnlineAreaId AreaId, EntryId Id)
    {
        std::shared_ptr<Area> AreaInstance = FindOrCreateArea(AreaId);
        if (auto iter = AreaInstance->Entries.find(Id); iter != AreaInstance->Entries.end())
        {
    ...
    std::vector<std::shared_ptr<ValueType>> GetRandomSet(DS3_OnlineAreaId AreaId, int MaxCount)
    {
        std::shared_ptr<Area> AreaInstance = FindOrCreateArea(AreaId);
    ...
    std::vector<std::shared_ptr<ValueType>> GetRecentSet(DS3_OnlineAreaId AreaId, int MaxCount, std::function<bool(std::shared_ptr<ValueType>)> FilterCallback)
    {
        std::vector<std::shared_ptr<ValueType>> Result;
    ...
    bool Contains(DS3_OnlineAreaId AreaId, EntryId Id)
    {
        return Find(AreaId, Id) != nullptr;
    }

    bool Add(DS3_OnlineAreaId AreaId, EntryId Id, std::shared_ptr<ValueType> Value)
    {
        std::shared_ptr<Area> AreaInstance = FindOrCreateArea(AreaId);
        if (auto iter = AreaInstance->Entries.find(Id); iter != AreaInstance->Entries.end())
        {
...
private:
    std::unordered_map<DS3_OnlineAreaId, std::shared_ptr<Area>> AreaMap;
